...
<body>
    <div class="container">
        ...
        if (isset($_POST["submit"])) {
            ...
            if (empty($email) || empty($password) || empty($passwordRepeat) || empty($lastName) || empty($firstName) || empty($sex) || empty($blockNo) || empty($lotNo) || empty($street) || empty($barangay) || empty($city)) {
            ...
            $sqlCheckEmail = "SELECT * FROM customer_information_tbl WHERE email = ?";
            ...
            if (count($errors) > 0) {
                ...
                $sqlInsertUser = "INSERT INTO customer_information_tbl (username, email, password) VALUES (?, ?, ?)";
                $stmtInsertUser = mysqli_stmt_init($conn);
                ...
                if (mysqli_stmt_prepare($stmtInsertUser, $sqlInsertUser)) {
                    ...
                    // Insert user information into customer_information_tbl
                    $sqlInsertInfo = "INSERT INTO customer_information_tbl (userName, email, password, lastName, firstName, middleName, sex, blockNo, lotNo, street, barangay, city) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmtInsertInfo = mysqli_stmt_init($conn);

                    if (mysqli_stmt_prepare($stmtInsertInfo, $sqlInsertInfo)) {
                        mysqli_stmt_bind_param($stmtInsertInfo, "ssssssssssss", $userName, $email, $passwordHash, $lastName, $firstName, $middleName, $sex, $blockNo, $lotNo, $street, $barangay, $city);
                        mysqli_stmt_execute($stmtInsertInfo);
                        echo "<div class='alert alert-success'>You are registered successfully.</div>";
                    } else {
                        die("Error in preparing SQL statement to insert user information");
                    }
                } else {
            ...
            mysqli_stmt_close($stmtInsertInfo);
        }
        ?>
        ...
        <div>
        <div><p>Already Registered <a href="customer_login.php">Login Here</a></p></div>
